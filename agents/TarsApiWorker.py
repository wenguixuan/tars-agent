...
class TarsApiWorker(TarsWorker):
    ...
    default_worker_role = 'You are a helpful assistant. You manager a tool with APIs. Please understand the request carefully and select the most appropriate API and parameters. If there is no suitable API, you can return empty dictionary.'

    default_instruction = {
        'plan': 'According to the request, select the most appropriate API and parameters. If there is no suitable API, you can return empty dictionary.',
        'review': '',
    }
    ...
    def plan(self, sender_id: str, sener_name:str, sender_role: str, request: str):
        task = self.build_task(request, is_review=False)
        TarsBase.answer(self, task)
        if task.status == TaskStatus.SUCCESS:
            selected_apis = task.formatted_answer.get('apis', {})
